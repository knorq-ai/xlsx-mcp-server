...
|------|-----------|------------|
| 操作あたりの出力トークン | **82–91% 削減** | ベースライン (エージェントがコード全体を生成) |
| 操作あたりのコスト | **69–83% 削減** | ベースライン |
| 損益分岐点 | **2 操作** | — |
| デバッグ反復 | なし (入力バリデーション済み) | 平均 ~1.5 回/タスク |

削減の主因は **コード生成の省略** である。出力トークンは入力トークンの 5 倍の単価であるため、MCP ツール呼び出し (~50 tokens の構造化パラメータ) と、Python コード生成 (~300–600 出力トークン/操作: import、ファイル操作、API 呼び出し、エラー処理) の差が大きい。

特に書式設定操作で最大の削減 (83%) が得られる。openpyxl のスタイル API (`PatternFill`, `Border`, `Side`, `Font`) が冗長なためである。
